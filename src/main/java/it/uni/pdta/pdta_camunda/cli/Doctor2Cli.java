 ...
 *
 * Comportamento:
 * 1. Polling automatico (ogni 5s) per task "Revisione Medico GOM 2"
...
public class Doctor2Cli implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        System.out.println(colorize(">>> Monitoraggio task 'Revisione Medico GOM 2'...", Color.CYAN));
        ...
        while (true) {
            try {
                // Cerca task GOM2 con state CREATED
                List<CamundaRestClient.UserTask> tasks = restClient.searchTasks(Map.of());

                CamundaRestClient.UserTask gom2Task = null;
                for (var task : tasks) {
                    if (task.name.equals("Revisione Medico GOM 2") && task.state.equals("CREATED")) {
                        gom2Task = task;
                        break;
                    }
                }
                ...
                if (gom2Task != null) {
                    ...
                    if (!taskFound) {
                        System.out.print("\r" + colorize("In attesa di task GOM2... (polling ogni " +
    ...
    private void processTask(CamundaRestClient.UserTask task) throws Exception {
        System.out.println("\r" + colorize("✓ Trovato task GOM2 in attesa!", Color.GREEN) + "                    ");
        System.out.println();
        ...
        System.out.println(colorize("=== AUTENTICAZIONE MEDICO GOM2 ===", Color.YELLOW));
        ...
        boolean gom2Approved = !approved.equalsIgnoreCase("n");

        System.out.print("Note di validazione: ");
        String gom2Note = scanner.nextLine().trim();
        if (gom2Note.isEmpty()) {
            gom2Note = gom2Approved ?
                    "Paziente idoneo per PDTA - GOM2" :
                    "Paziente non idoneo per PDTA - GOM2";
        }

        // Riepilogo
        System.out.println();
        System.out.println(colorize("=== RIEPILOGO ===", Color.YELLOW));
        System.out.println("Approvazione: " + gom2Approved);
        System.out.println("Note: " + gom2Note);
        ...
        // Completa task
        System.out.println(">>> Completamento task GOM2...");
        Map<String, Object> taskVariables = new HashMap<>();
        taskVariables.put("gom2Approved", gom2Approved);
        taskVariables.put("gom2Note", gom2Note);
        taskVariables.put("gom2SecretCode", secretCode);

        restClient.completeTask(task.userTaskKey, taskVariables);

        System.out.println(colorize("✓ Revisione GOM2 completata con successo!", Color.GREEN));
        System.out.println();
        System.out.println("╔════════════════════════════════════════════════╗");
        System.out.println("║        VALIDAZIONE GOM2 COMPLETATA             ║");
        System.out.println("╚════════════════════════════════════════════════╝");
        System.out.println();
        System.out.println(colorize("Il processo ora procederà con la firma digitale GOM2", Color.CYAN));
        System.out.println(colorize("e la generazione del documento PDTA finale.", Color.CYAN));
    }

    private void printHeader() {
        System.out.println();
        System.out.println("╔════════════════════════════════════════════════╗");
        System.out.println("║        Sistema PDTA - Medico 2 (GOM2)          ║");
        System.out.println("╚════════════════════════════════════════════════╝");
        System.out.println();
    }
